...
#
# Launch test file for ros_func_tests (ROSFunctionalityTests).
# Automatically starts a minimal mock-hardware simulation (no Gazebo required)
# before running the GTest binary.
#
# Usage (manual):
#   ros2 launch cartesian_impedance_controller ros_func_tests.launch.py
#
    ...
    return (
        launch.LaunchDescription(
            [
                ...
                TimerAction(period=10.0, actions=[ros_func_tests]),
                launch_testing.actions.ReadyToTest(),
            ]
        ),
        {"ros_func_tests": ros_func_tests},
    )


class TestROSFuncTestsOutput(unittest.TestCase):
    def test_exit_code(self, proc_info, ros_func_tests):
        proc_info.assertWaitForShutdown(process=ros_func_tests, timeout=180)


@launch_testing.post_shutdown_test()
class TestROSFuncTestsAfterShutdown(unittest.TestCase):
    def test_exit_codes(self, proc_info, ros_func_tests):
        launch_testing.asserts.assertExitCodes(proc_info, process=ros_func_tests)
